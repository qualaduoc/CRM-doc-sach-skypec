    ...
    for (const flight of flightsToSync) {
      ...
      await db.run(`
        ...
        ON CONFLICT(flight_no, date) DO UPDATE SET
          ...
          gate = excluded.gate,
          truck_no = CASE WHEN excluded.truck_no IS NOT NULL AND excluded.truck_no != '' THEN excluded.truck_no ELSE fms_flights_live.truck_no END,
          driver_name = CASE WHEN excluded.driver_name IS NOT NULL AND excluded.driver_name != '' THEN excluded.driver_name ELSE fms_flights_live.driver_name END,
          operator_name = CASE WHEN excluded.operator_name IS NOT NULL AND excluded.operator_name != '' THEN excluded.operator_name ELSE fms_flights_live.operator_name END,
          standby_fuel = excluded.standby_fuel,
          fuel_order = CASE WHEN excluded.fuel_order IS NOT NULL AND excluded.fuel_order != '' AND excluded.fuel_order != '0' THEN excluded.fuel_order ELSE fms_flights_live.fuel_order END,
          status = CASE WHEN excluded.status IS NOT NULL AND excluded.status != '' THEN excluded.status ELSE fms_flights_live.status END,
