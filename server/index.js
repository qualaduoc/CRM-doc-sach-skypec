  ...
  try {
    ...
    const rows = await db.all(`
      SELECT 
        ...
        fo.dep_arr,
        COALESCE(NULLIF(fo.standby_fuel, ''), NULLIF(fl.standby_fuel, ''), '') as standby_fuel,
        COALESCE(NULLIF(fo.fuel_order, ''), NULLIF(fl.fuel_order, ''), '') as fuel_order,
        fo.trip_fuel,
        fo.trip_time,
        fo.taxi_fuel,
        fo.alternate,
...
app.get('/api/fms/user-stats', authenticateToken, async (req, res) => {
  try {
    ...
    // 5. Truy vấn database lấy toàn bộ chuyến bay trong 3 mốc thời gian (kết hợp fms_schedules & fms_flights_live)
    const rows = await db.all(`
      SELECT 
