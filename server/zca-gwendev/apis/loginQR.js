...
// ─── Chrome version mới nhất (2026) ───
const CHROME_UA = '"Chromium";v="136", "Google Chrome";v="136", "Not?A_Brand";v="99"';
...
export async function loginQR(ctx, options, callback) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            console.log('[QR-DEBUG] getUserInfo response:', JSON.stringify(userInfo?.data ? { logged: userInfo.data.logged, hasInfo: !!userInfo.data.info } : userInfo));
            if (!userInfo || !userInfo.data)
                throw new ZaloApiError("Can't get account info. Response: " + JSON.stringify(userInfo));
            if (!userInfo.data.logged)
                throw new ZaloApiError("Can't login - session not logged. getUserInfo returned logged=false");
            resolve({
