...
export async function request(ctx, url, options, raw = false) {
    ...
    if (setCookieRaw && !raw) {
        // Ưu tiên dùng getSetCookie() (chuẩn mới, tách cookie chính xác)
        let cookieStrings = [];
        if (typeof response.headers.getSetCookie === 'function') {
            cookieStrings = response.headers.getSetCookie();
        } else {
            // Fallback: split thông minh - tránh cắt sai ở Expires (ví dụ: "Thu, 01 Jan")
            cookieStrings = setCookieRaw.split(/,(?=\s*[a-zA-Z_][a-zA-Z0-9_]*=)/);
        }
        for (const cookieStr of cookieStrings) {
            const trimmed = cookieStr.trim();
            if (!trimmed) continue;
            const parsed = toughCookie.Cookie.parse(trimmed);
            try {
                if (parsed)
    ...
    return (callback) => {
        return (ctx, api) => {
            ...
            const utils = {
                ...
                decodeAES(data, t) {
                    return decodeAES(ctx.secretKey, data, t);
                },
