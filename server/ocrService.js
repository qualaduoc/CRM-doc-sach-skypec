...
async function testSingleGeminiKey(apiKey) {
      ...
      res.on('end', () => {
        if (res.statusCode === 200) {
          ...
          try {
            ...
            resolve({ success: false, error: errJson.error ? errJson.error.message : body });
          } catch(e) {
            resolve({ success: false, error: `Lỗi HTTP ${res.statusCode}` });
          }
        }
      });
    });
